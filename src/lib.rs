...
use errors::{ConvToWbfsError, CopyWbfsFileError, ParseImageError};

extern "C" {
    fn conv_to_wbfs(filename: *const c_char, dest_dir: *const c_char);
}

fn get_id_and_title(file: &mut File) -> Result<(String, String), ParseImageError> {
    // read the id
    let mut id = [0u8; 0x6];
...
pub fn conv_to_wbfs_wrapper(src: &Path, dest: &Path) -> Result<(), ConvToWbfsError> {
...
pub fn copy_wbfs_file(src: &Path, dest: &Path) -> Result<(), CopyWbfsFileError> {
    let mut file = File::open(src)?;
    ...
    // check if the file is a wbfs file
    {
        ...
        if magic != [0x57, 0x42, 0x46, 0x53] {
            return Err(CopyWbfsFileError::NotAWbfsFile);
        }
    }
